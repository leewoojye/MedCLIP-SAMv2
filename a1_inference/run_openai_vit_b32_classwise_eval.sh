#!/usr/bin/env bash
# Run OpenAI CLIP ViT-B/32 evaluation with cache writes confined to a1_inference.
set -euo pipefail
...
# Use an A6000 by default; an explicit CUDA_VISIBLE_DEVICES setting wins.
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}"

exec "${PYTHON_BIN}" "${SCRIPT_DIR}/openai_vit_b32_classwise_eval.py" "$@"

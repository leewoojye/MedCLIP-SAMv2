...
class DpoCsvDataset(Dataset):
    def __init__(self, input_filename, transforms, img_key, img_neg_key, caption_key, caption_neg_key, sep="\t", tokenizer=None):
        logging.debug(f'Loading DPO csv data from {input_filename}.')
        df = pd.read_csv(input_filename, sep=sep)

        self.images = df[img_key].tolist()
        self.images_neg = df[img_neg_key].tolist()
        self.captions = df[caption_key].tolist()
        self.captions_neg = df[caption_neg_key].tolist()
        self.transforms = transforms
    ...
    def __getitem__(self, idx):
        images = self.transforms(Image.open(str(self.images[idx])))
        images_neg = self.transforms(Image.open(str(self.images_neg[idx])))
        texts = self.tokenize([str(self.captions[idx])])[0]

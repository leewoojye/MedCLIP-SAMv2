...
def main(args):
    ...
    print("Using Text Anchors: 'healthy breast' - 'tumor breast'")
    phrases = ["tumor breast", "healthy breast", "tumor brain"]
    embs = get_text_embedding(bio_tokenizer, bio_model, phrases, device=device)
    
    v_tumor_breast = embs[0:1] # (1, seq_len, hidden_size)
    v_healthy_breast = embs[1:2]
    v_tumor_brain = embs[2:3]
    
    # Delta V = Healthy - Tumor (The "Healing" Shift)
    shift_vector = v_healthy_breast - v_tumor_breast
    
    # New Condition = Tumor Brain + Alpha * Shift
    alpha = args.alpha
    print(f"Injecting shift vector with alpha = {alpha}")
    target_embedding = v_tumor_brain + (alpha * shift_vector)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image", type=str, required=True, help="Path to original brain tumor MRI")
    parser.add_argument("--mask", type=str, required=True, help="Path to mask (white=tumor area to paint over)")
    parser.add_argument("--out", type=str, default="zero_shot_translation/output.png")

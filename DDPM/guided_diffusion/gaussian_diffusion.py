...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        :param clip_denoised: if True, clip the denoised signal into [0, 1].
        ...
        for i in indices:
            ...
            with th.no_grad():
                if img.shape[1] != shape[1]:
                    img = torch.cat(
                        (org_MRI, img), dim=1
                    )  # concatenate condition channels back at every step
    ...
    def training_losses_segmentation(
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if loss_weights is not None:
                weights = loss_weights.to(squared_error.dtype)
                numer = (squared_error * weights).sum(
                    dim=list(range(1, len(squared_error.shape)))
                )
                denom = weights.sum(dim=list(range(1, len(weights.shape)))).clamp(
                    min=1e-8
                )
                terms["mse"] = numer / denom
            else:

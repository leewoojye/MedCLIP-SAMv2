...
output_dir = os.path.join(base_dir, "BUSI_aug")
...
# ---------------------------------------------------------
# 1. Gather BUSI original image IDs
# ---------------------------------------------------------
...
train_items = [("busi", x[0], x[1]) for x in busi_train]
test_items = [("busi", x[0], x[1]) for x in busi_test]
...
# Since they liked test outputs to be labeled as 'busi_test_dpo.csv' etc in previous dirs, we'll keep the simple train/test format
train_df.to_csv(os.path.join(output_dir, "busi_train_dpo.csv"), index=False)
test_df.to_csv(os.path.join(output_dir, "busi_test_dpo.csv"), index=False)

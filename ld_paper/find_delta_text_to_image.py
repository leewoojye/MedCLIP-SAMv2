...
Where:
  D_θ(δ) = distribution of images generated by SD with latent drift δ
  D_GT   = distribution of target medical images (real tumor dataset)
"""
...
# ---------------------------------------------------------------------------
# Custom pipeline with latent drift
# ---------------------------------------------------------------------------

class StableDiffusionWithLatentDrift(StableDiffusionPipeline):
    """SD pipeline with latent drift in reverse process."""
    
    def __init__(self, *args, delta: float = 0.0, **kwargs):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """Generate image with latent drift applied."""
        ...
        for t in tqdm(self.scheduler.timesteps, desc="Denoising"):
            ...
            # In reverse process: pθ(xt-1|xt) = N(xt-1; µθ(xt,t) + δ, Σθ(xt,t))
            if abs(self.delta) > 1e-9:
                noise_pred = noise_pred + self.delta
...
def generate_with_latent_drift(
    ...
    """Generate images with latent drift δ."""
    # Update pipeline delta
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Target domain D_GT (real tumor images)",
    )
    parser.add_argument(
        "--prompt",
        type=str,
...
def main():
    ...
    # Load model
    print("Loading model…")
    pipeline = StableDiffusionWithLatentDrift.from_pretrained(
        args.model_path,
    ...
    if not reference_images:
        raise RuntimeError(f"No target images found in {args.reference_data_dir}")

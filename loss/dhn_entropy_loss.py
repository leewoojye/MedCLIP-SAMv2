    ...
    def forward(self, image_features, text_features, logit_scale=None, logit_bias=None, attn_maps=None, output_dict=False):
        ...
        if self.entropy_weight > 0 and attn_maps is not None:
             ...
             if attn.dim() == 4: # [B, H, L, L]
                 ...
                 # Averaging attention maps first (as before) forces all heads to agree on one spot,
                 # which can cause "blurriness" if heads try to look at different valid features.
                 # Minimizing entropy per head allows each head to be sharp individually.
                 attn_cls = attn[:, :, 0, 1:] # [B, H, L-1]
